...
clusterprob = dict()
wicketprob = dict()
with open(r'C:\Users\Shashidhar\5th sem\Big Data\Project\Newstuff\CSV\bowlclusters.csv','rt')as a:
	data = csv.reader(a)
	for row in data:
		name = row[0].strip("'")
		c = row[1]
		bowlclusters[name] = c
a.close()
with open(r'C:\Users\Shashidhar\5th sem\Big Data\Project\Newstuff\CSV\batclusters.csv','rt')as b:
	data = csv.reader(b)
	for row in data:
		name = row[0].strip("'")
		c = row[1]
...
with open(r'C:\Users\Shashidhar\5th sem\Big Data\Project\Newstuff\CSV\pvp.csv','rt')as c:
	...
	for row in data:
		...
		pvp[bat][bowl] = prob
c.close()
with open(r'C:\Users\Shashidhar\5th sem\Big Data\Project\Newstuff\CSV\clusterprob.csv','rt')as d:
	data = csv.reader(d)
	for row in data:
		bid = row[0]
		bowlid = row[1]
